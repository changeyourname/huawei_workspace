...
var table = maker("table"), tr = maker("tr"), th = maker("th"), td = maker("td"), fs = maker("fieldset"); //, lg = maker("legend");
...
function display_processor (fd, fileIndex, x) {
    ...
    if (x.getAttribute ('procdoc')) tab.appendChild(tr([td("Full Documentation"), td(x.getAttribute ('procdoc'))]));
