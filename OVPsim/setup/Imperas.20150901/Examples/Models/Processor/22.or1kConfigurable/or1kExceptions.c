...
static vmiNetPort netPorts[] = {
    ...
    {"intr4", vmi_NP_INPUT, (void*)16,  externalInterrupt },
    {"intr5", vmi_NP_INPUT, (void*)32,  externalInterrupt },
    {"intr6", vmi_NP_INPUT, (void*)64,  externalInterrupt },
    {"intr7", vmi_NP_INPUT, (void*)128, externalInterrupt },
    VMI_END_NET_PORTS
};

//
// Get the next net port.
// Processor configuration set the number of ports implemented
//
VMI_NET_PORT_SPECS_FN(or1kGetNetPortSpec) {
    ...
    or1kP or1k = (or1kP)processor;
    prev++;
    Uns32 num = prev - netPorts;
    if (prev->name && num < or1k->numExtInts) {
        return prev;
    }
    return 0;
}
