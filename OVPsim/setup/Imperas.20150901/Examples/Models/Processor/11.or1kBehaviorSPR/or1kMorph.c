...
// known at morph time
//
static void vmic_MTSPR(or1kP or1k, Uns32 thisPC, Uns32 ra, Uns32 rb, Uns32 k) {
...
// known at morph time
//
static void vmic_MFSPR(or1kP or1k, Uns32 thisPC, Uns32 rd, Uns32 ra, Uns32 k) {
...
// be zero at morph time
//
static void doMFSPR_ra_0(Uns32 thisPC, Uns32 rd, Uns32 sprNum) {
    ...
    switch(sprNum) {
        ...
        case SPR_OFF(SPR_SYS,SYS_SR):
            vmimtArgProcessor();
            vmimtArgUns32(rd);
            vmimtCall((vmiCallFn)vmic_MFSPR_SYS_SR);
            break;

        case SPR_OFF(SPR_SYS,SYS_EPC):
            vmimtArgProcessor();
            vmimtArgUns32(rd);
            vmimtCall((vmiCallFn)vmic_MFSPR_SYS_EPC);
            break;

        case SPR_OFF(SPR_SYS,SYS_EEAR):
            vmimtArgProcessor();
            vmimtArgUns32(rd);
            vmimtCall((vmiCallFn)vmic_MFSPR_SYS_EEAR);
            break;

        case SPR_OFF(SPR_SYS,SYS_ESR):
            vmimtArgProcessor();
            vmimtArgUns32(rd);
            vmimtCall((vmiCallFn)vmic_MFSPR_SYS_ESR);
            break;
...
//
static void doMFSPR(Uns32 instr, Uns32 thisPC) {
...
//
static OR1K_DISPATCH_FN(morphMTSPR) {doMTSPR(instr, thisPC);}
static OR1K_DISPATCH_FN(morphMFSPR) {doMFSPR(instr, thisPC);}
