...
static void channelThread(void *user)
{
    Uns32 ch = (Uns32) user;
    for (;;) {
        if (diagnosticLevel >= 2) bhmMessage("I", "DMAC", "ch %u waiting", ch);
        DMAState.ch[ch].busy = False;

        bhmWaitEvent(DMAState.ch[ch].start);
        if (DMAState.inReset) {
            if (diagnosticLevel >= 2) bhmMessage("I", "DMAC", "In reset");
        } else {
            DMAState.ch[ch].busy = True;
            if (diagnosticLevel >= 2) bhmMessage("I", "DMAC", "ch %u running", ch);
