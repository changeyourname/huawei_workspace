...
// simulated instruction and return from simulator invocation.
//
...
//
void vmirtWriteNetPort(vmiProcessorP processor, Uns32 handle, Uns32 value);

//
// Read a value from a processor net port
//
Uns32 vmirtReadNetPort(vmiProcessorP processor, Uns32 handle);
...
//
// Get a pointer to a NULL-terminated string in simulated memory.
//
...
// instructions that are enabled by processor mode bits
//
...
//
// Return the number of processor instructions executed.
//
Uns64 vmirtGetICount(vmiProcessorP processor);

//
// Schedule an interrupt after iDelta instructions using the processor implicit
// timer (callback specified as the icountExceptCB member of the processor
// vmiIASAttr structure).
//
...
//     scale = 1:   timer increments once every instruction.
//     scale = 10:  timer increments once every 10 instructions.
...
// 'virtualLowAddr' in the virtual domain.
//
...
//
void vmirtConnNotifyGet(vmiConnOutputP conn, vmiConnUpdateFn updateCB);

//
// Call 'updateCB' when data is written into 'conn'.
//
void vmirtConnNotifyPut(vmiConnInputP conn, vmiConnUpdateFn updateCB);
...
//
// Return a handle to a shared data object for the passed key (or NULL if no
// handle exists for that key)
//
...
//
// Return a handle to a shared data object for the passed key, creating a new
// handle if one with that name does not yet exist, and set the shared data
// object value
//
...
// The entry must not overlap any existing entries; if it does, the function
// returns NULL.
//
...
// In addition to the arguments specified using vmirtAddArg, the flags
// -get is recognized.
// If attrs contains the bit VMI_CT_HELP, then -help is recognized.
// If the defined command is used, commandCB will be called with the
// arguments parsed into an argValue array. The array does not
...
//
// Add an argument to a command parser. If the command is called with
// an argument matching the given name, it will be parsed.
// If ptr is non-null, its value will be updated by the value parsed.
// Note that if ptr  is pointing to a string,
// the old string will be free()'d before a new string is malloc()'d
//
vmiArgP vmirtAddArg(
    vmiCommandP     command,
    const char     *name,
...
void vmirtAddArgEnum(
    ...
    const char     *help,     // not used
    Uns32           value     // not used
);
