...
class icmPlatform : icmCpuManagerItem {
    ...
    public:
        ...
        /// Get a callback on each user-defined attribute in each model.
        /// Can be used for verification.
        /// @param fn        User function to be called.
        /// @param user      User data passed to callback.

        void iterAllUserAttributes(icmAttrFn fn,  void *user) { icmIterAllUserAttributes(fn, user); }
...
public:
    ...
    /// Simulate this number of processor instructions
    /// @param instructions   Number of instructions.
    /// @return               Reason for stopping.
    /// Note that the simulation might complete prematurely for other reasons.

    icmStopReason simulate(Uns64 instructions);
