...
static void guard_assign (Uns8 *guard, ppmNetHandle handle, Uns8 value, char *name) {
    if (*guard != value) {
        if (BHM_DIAG_LOW) bhmMessage("I", PREFIX, "guard_assign() PIN %s <= %d\n", name, value);
...
void evaluate () {
    ...
    if (BHM_DIAG_LOW) bhmMessage("I", PREFIX, "evaluate() irr=0x%02x mask=0x%02x irr_postmask=0x%02x\n",
    ...
    if (irr_postmask) {
        for(i=0; i<8; i++) {
            ...
            p = reg.priority[i];
            if (irq && (p < maxp)) {
                if (BHM_DIAG_LOW) bhmMessage("I", PREFIX, "evaluate() Candidate irq=%d priority=%d\n", i, p);
                maxp = p;
                irqhi = i;
            }
        }
        if (BHM_DIAG_LOW) bhmMessage("I", PREFIX, "evaluate() Selected irq=%d priority=%d\n", irqhi, maxp);
    ...
    if (pin.intack == True) {
        ...
        if (irqhi < 8) {
            ...
            if (BHM_DIAG_LOW) bhmMessage("I", PREFIX, "evaluate() INTACK irq=%d irr=0x%02x isr=0x%02x\n", irqhi, reg.irr, reg.isr);
    ...
    if (pin.eoi == True || (pin.intack && reg.icw4.bits.aeoi)) {
        if (BHM_DIAG_LOW) bhmMessage("I", PREFIX, "evaluate(0) EOI irr=0x%02x isr=0x%02x\n", reg.irr, reg.isr);

        // Specific or non specific ?
        if (reg.ocw2.bits.sl) {
            if (BHM_DIAG_LOW) bhmMessage("I", PREFIX, "Performing Specific EOI L=%d", reg.ocw2.bits.l);
            Uns8 mask = 1 << reg.ocw2.bits.l;
            reg.isr &= ~mask;

        // Non-specific highest is bit
        } else {
            if (BHM_DIAG_LOW) bhmMessage("I", PREFIX, "Performing Non-Specific EOI");
            for (i=0; i<8; i++) {
                Uns8 mask = 1 << i;
                if (reg.isr & mask) {
                    reg.isr &= ~mask;
        ...
        if (BHM_DIAG_LOW) bhmMessage("I", PREFIX, "evaluate(1) EOI irr=0x%02x isr=0x%02x\n", reg.irr, reg.isr);
    ...
    if (D4 & data) {
        ...
        if (nextState == ready) {
            ...
            if (!(D3 & data)) {
                ...
                if (BHM_DIAG_LOW) ocw2Print();
            } else {
                reg.ocw3.byte = data;
                if (BHM_DIAG_LOW) ocw3Print();
            }
        } else {
            if (BHM_DIAG_LOW) {
...
PPM_REG_WRITE_CB(CB_iow1) {
    ...
    if (nextState == ready) {
        reg.ocw1.byte = data;
        if (BHM_DIAG_LOW) ocw1Print();
    } else {
        switch (nextState) {
        case icw2:
            reg.icw2.byte = data;
            if (BHM_DIAG_LOW) icw2Print();
...
//
static void CB_ir(Uns32 value, void *user)
{

    Uns32 index = (Uns32) user;
    if(BHM_DIAG_LOW)
        bhmMessage("I", PREFIX, "EVENT %f: ir%d=%d", bhmGetCurrentTime(), index, value);
...
static void getSpen() {
    ...
    Bool setSpen = bhmStringAttribute("spen", spenString, sizeof(spenString));
    if(setSpen){
...
static void netPortConnections(void) {
    ...
    for(i=0; i < 8; i++) {
        if(irHandle[i]) {
            ...
            ppmInstallNetCallback(irHandle[i], CB_ir, (void*)i);
            if(BHM_DIAG_MEDIUM) {
