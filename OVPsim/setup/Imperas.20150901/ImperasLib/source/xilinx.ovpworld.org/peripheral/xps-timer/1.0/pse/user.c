...
PPM_REG_READ_CB(ReadTCR0) {
    ...
    return plb_REG_data.TCR0.value;
}
...
//    return GETBE(plb_REG_data.TCR1.value);
    return plb_REG_data.TCR1.value;
}
...
//    }
    ...
    //
    // Enable All Timers - mirrored ?
    //
    plb_REG_data.TCSR0.bits.ENALL = TCSR.bits.ENALL;
    plb_REG_data.TCSR1.bits.ENALL = TCSR.bits.ENALL;
    if (TCSR.bits.ENALL) {
        TCSR.bits.ENT = 1;
