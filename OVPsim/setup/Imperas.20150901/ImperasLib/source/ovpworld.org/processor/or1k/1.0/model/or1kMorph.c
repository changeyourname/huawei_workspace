...
//
static void vmic_MTSPR_PIC_MR(or1kP or1k, Uns32 rb) {
    or1kSetPICMR(or1k, or1k->regs[rb]);
}
...
//
static void vmic_MFSPR_PIC_MR(or1kP or1k, Uns32 rd) {
    or1k->regs[rd] = or1k->PICMR;
}

//
// Run-time implementation of mfspr with picsr register
//
static void vmic_MFSPR_PIC_SR(or1kP or1k, Uns32 rd) {
    or1k->regs[rd] = or1k->PICSR;
}

//
// Get from connection, block if data not available
//
static void vmic_ConnPutOrBlock(or1kP or1k, Uns32 thisPC, Uns32 rb) {

    if(!vmirtConnPut(or1k->outputConn, &or1k->regs[rb])) {
        ...
        vmirtConnNotifyGet(or1k->outputConn, restartAfterBlock);
    }
}
...
// into carry flag
//
static void vmic_ConnPut(or1kP or1k, Uns32 rb) {

    or1k->carryFlag = vmirtConnPut(or1k->outputConn, &or1k->regs[rb]);
}

//
// Put to connection, block if space not available
//
static void doConnPutOrBlock(Uns32 rb) {
    vmimtConnPutRB(
        OR1K_BITS,
        OR1K_CPU_REG(outputConn),
        OR1K_REG(rb),
        restartAfterBlock
    );
}
...
// into carry flag
//
static void doConnPut(Uns32 rb) {
    vmimtConnPutRNB(
        OR1K_BITS,
        OR1K_CPU_REG(outputConn),
        OR1K_REG(rb),
...
// Get from connection, block if data not available
//
static void vmic_ConnGetOrBlock(or1kP or1k, Uns32 thisPC, Uns32 rd, Bool peek) {

    if(!vmirtConnGet(or1k->inputConn, &or1k->regs[rd], peek)) {

        // halt this processor and re-execute this instruction on restart
        vmirtBlock((vmiProcessorP)or1k);

        // install notifier to be called on connection event
        vmirtConnNotifyPut(or1k->inputConn, restartAfterBlock);
    }
}
...
// into carry flag
//
static void vmic_ConnGet(or1kP or1k, Uns32 rd, Bool peek) {

    or1k->carryFlag = vmirtConnGet(or1k->inputConn, &or1k->regs[rd], peek);
}

//
// Get from connection, block if data not available
//
static void doConnGetOrBlock(Uns32 rd, Bool peek) {
    vmimtConnGetRB(
        OR1K_BITS,
        OR1K_REG(rd),
        OR1K_CPU_REG(inputConn),
        peek,
        restartAfterBlock
    );
}
...
// into carry flag
//
static void doConnGet(Uns32 rd, Bool peek) {
    vmimtConnGetRNB(
        OR1K_BITS,
        OR1K_REG(rd),
        OR1K_CPU_REG(inputConn),
...
static void vmic_IgnoreMTSPR(
    ...
    if(OR1K_ENABLE_FIFOS((vmiProcessorP)or1k)) {

        switch(sprNum) {

            case SPR_OFF(SPR_SYS,SYS_FIFO_B):
                vmic_ConnPutOrBlock(or1k, thisPC, rb);
                return;

            case SPR_OFF(SPR_SYS,SYS_FIFO_NB):
                vmic_ConnPut(or1k, rb);
                return;
        }
    }
...
static void vmic_IgnoreMFSPR(
    ...
    if(OR1K_ENABLE_FIFOS((vmiProcessorP)or1k)) {
        ...
        switch(sprNum) {
            ...
            case SPR_OFF(SPR_SYS,SYS_FIFO_B):
                vmic_ConnGetOrBlock(or1k, thisPC, rd, False);
                return;

            case SPR_OFF(SPR_SYS,SYS_FIFO_NB):
                vmic_ConnGet(or1k, rd, False);
                return;

            case SPR_OFF(SPR_SYS,SYS_PEEK_B):
                vmic_ConnGetOrBlock(or1k, thisPC, rd, True);
                return;

            case SPR_OFF(SPR_SYS,SYS_PEEK_NB):
                vmic_ConnGet(or1k, rd, True);
                return;
        }
    }
...
static void vmic_MTSPR_SM(
    ...
    switch(sprNum) {
        ...
        default:
            vmic_IgnoreMTSPR(or1k, thisPC, ra, rb, k, sprNum);
            break;
    }
}
...
static void vmic_MFSPR_SM(
    ...
    switch(sprNum) {
        ...
        case SPR_OFF(SPR_PIC,PIC_MASK):
            vmic_MFSPR_PIC_MR(or1k, rd);
            break;

        case SPR_OFF(SPR_PIC,PIC_STAT):
            vmic_MFSPR_PIC_SR(or1k, rd);
            break;

        default:
            vmic_IgnoreMFSPR(or1k, thisPC, rd, ra, k, sprNum);
            break;
    }
}
...
//
static void doMTSPR_ra_0_Default(
    or1kP or1k,
    Uns32 thisPC,
    Uns32 rb,
    Uns32 sprNum
...
// Emit default action for unimplemented mfspr
//
static void doMFSPR_ra_0_Default(
    or1kP or1k,
    Uns32 thisPC,
    Uns32 rd,
    Uns32 sprNum
...
//
static void doMTSPR_ra_0_SM(or1kP or1k, Uns32 thisPC, Uns32 rb, Uns32 sprNum) {
    ...
    switch(sprNum) {
        ...
        default:
            doMTSPR_ra_0_Default(or1k, thisPC, rb, sprNum);
            break;
    }
}
...
// be zero at morph time. Function called in SUPERVISOR MODE ONLY
//
static void doMFSPR_ra_0_SM(or1kP or1k, Uns32 thisPC, Uns32 rd, Uns32 sprNum) {
    ...
    switch(sprNum) {
        ...
        case SPR_OFF(SPR_PIC,PIC_MASK):
            vmimtMoveRR(OR1K_BITS, OR1K_REG(rd), OR1K_PICMR);
            break;

        case SPR_OFF(SPR_PIC,PIC_STAT):
            vmimtMoveRR(OR1K_BITS, OR1K_REG(rd), OR1K_PICSR);
            break;

        default:
            doMFSPR_ra_0_Default(or1k, thisPC, rd, sprNum);
            break;
    }
}
...
//
static void doMTSPR_ra_0_UM(or1kP or1k, Uns32 thisPC, Uns32 rb, Uns32 sprNum) {
...
// be zero at morph time. Function called in USER MODE ONLY
//
static void doMFSPR_ra_0_UM(or1kP or1k, Uns32 thisPC, Uns32 rd, Uns32 sprNum) {
...
static void doMTSPR(or1kP or1k, Uns32 instr, Uns32 thisPC) {
    ...
    if(ra==0) {
        ...
        if(IN_SUPERVISOR_MODE(or1k)) {
            doMTSPR_ra_0_SM(or1k, thisPC, rb, k);
        } else {
            doMTSPR_ra_0_UM(or1k, thisPC, rb, k);
        }
...
static void doMFSPR(or1kP or1k, Uns32 instr, Uns32 thisPC) {
    ...
    if(rd==0) {
        ...
        // mode function, based on current mode setting in sr
        if(IN_SUPERVISOR_MODE(or1k)) {
            doMFSPR_ra_0_SM(or1k, thisPC, rd, k);
        } else {
            doMFSPR_ra_0_UM(or1k, thisPC, rd, k);
        }
