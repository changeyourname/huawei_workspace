...
static vmidDecodeTableP createDecodeTable(void) {
    ...
    DECODE_ENTRY(0, MTSPR, "|110000..........................|");
    DECODE_ENTRY(0, MFSPR, "|101101..........................|");
...
static or1kInstructionType decode(Uns32 instruction) {
    ...
    switch(type) {
        case OR1K_IT_SF:
        case OR1K_IT_SFI:
            if(!getCmpInfo(OP_CMPOP(instruction))->name) {
                type = OR1K_IT_LAST;
            }
        default:
            break;
...
const char *or1kDecodeCmpName(Uns32 instruction) {
    switch(decode(instruction)) {
        case OR1K_IT_SF:
        case OR1K_IT_SFI:
            return getCmpInfo(OP_CMPOP(instruction))->name;
        default:
            return 0;
    }
}
...
vmiCondition or1kDecodeCmpCondition(Uns32 instruction) {
    switch(decode(instruction)) {
        case OR1K_IT_SF:
        case OR1K_IT_SFI:
            return getCmpInfo(OP_CMPOP(instruction))->cond;
        default:
            return vmi_COND_Z;
    }
}
