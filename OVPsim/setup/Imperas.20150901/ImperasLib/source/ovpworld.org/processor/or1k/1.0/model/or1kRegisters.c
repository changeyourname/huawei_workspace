...
static VMI_REG_WRITE_FN(writeSR) {
    or1kP or1k = (or1kP)processor;
    or1kSetSR(or1k, *(Uns32*)buffer);
    return True;
}

//
// Read callback for PICMR
//
static VMI_REG_READ_FN(readPICMR) {
    or1kP or1k = (or1kP)processor;
    *(Uns32*)buffer = or1k->PICMR;
    return True;
}
...
static VMI_REG_WRITE_FN(writePICMR) {
    or1kP or1k = (or1kP)processor;
    or1kSetPICMR(or1k, *(Uns32*)buffer);
    return True;
}
...
static VMI_REG_WRITE_FN(writeTTCR) {
    or1kP or1k = (or1kP)processor;
    or1kSetTTCR(or1k, *(Uns32*)buffer);
    return True;
}

//
// Read callback for TTMR
//
static VMI_REG_READ_FN(readTTMR) {
    or1kP or1k = (or1kP)processor;
    *(Uns32*)buffer = or1k->TTMR;
    return True;
}
...
static const vmiRegInfo registers[] = {
    ...
    OR1K_REG_INFO("PICMR", 101, vmi_REG_NONE, OR1K_BITS, OR1K_CB_REG_RW (PICMR,     OR1K_GROUP(SYSTEM)),      "PIC mask register"),
    ...
    OR1K_REG_INFO("TTMR",  104, vmi_REG_NONE, OR1K_BITS, OR1K_CB_REG_RW (TTMR,      OR1K_GROUP(SYSTEM)),      "tick timer mode register"),
