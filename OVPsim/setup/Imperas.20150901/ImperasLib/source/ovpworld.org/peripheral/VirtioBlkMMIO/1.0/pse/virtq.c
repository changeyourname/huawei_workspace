...
//
Addr  virtqBase     = 0;
Uns16 lastAvailSeen = INVALID_IDX;
...
static Bool readVirtq(Uns32 offset, Uns32 bytes, void *data) {
    ...
    } else if (!readSimMemory(virtqBase+offset, bytes, data)) {

        bhmMessage(
            "W", PREFIX"_USM",
            "Unable to read %d bytes at offset %d of virtq at %s",
            bytes, offset, uns64ToString(virtqBase)
        );
        return False;
...
Bool writeVirtq(Uns32 offset, Uns32 bytes, void *data) {
    ...
    } else if (!writeSimMemory(virtqBase+offset, bytes, data)) {
...
Uns16 getNextAvailEntry() {
    ...
    if (
        ...
        (availIndex != lastAvailSeen) &&
        (readVirtq(AVAIL_OFFSET(lastAvailSeen), sizeof(entry), &entry))
       ) {
...
Bool getVirtqDesc(Uns16 index, virtqDescP desc) {
    ...
    if (index != INVALID_IDX && readVirtq(DESC_OFFSET(index), DESC_SIZE, desc)) {
        ...
        if (DIAG_MED) {
            bhmMessage(
                "I", PREFIX"_DTE",
                "virtqDesc[%d]: Addr=%s, Len=%d, Flags=0x%x, Next=%d",
                index,
...
Bool writeVirtqBuffer(virtqDescP desc, Uns32 bytes, void *data) {
    ...
    while (bytes > 0) {
        ...
        if ((avail==0) && !getVirtqDesc(nextDesc(desc), desc)) {
            bhmMessage("W", PREFIX"_BOW", "Buffer overflow on write");
            return False;
        }
