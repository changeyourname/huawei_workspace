...
static int loadHexFile(vmiosObjectP object, const char *fileName, Bool swap) {
    ...
    while (!feof(fp) && !ferror(fp)) {
        inBuf[0] = '\0';
        fgets(inBuf, MAX_LINE_LENGTH, fp);
...
static VMIOS_INTERCEPT_FN(flashSetDiag)
{
...
//
static VMIOS_INTERCEPT_FN(flashInit)
{
    Uns32 port_nameAddr;
    Uns32 fileToLoadAddr;
    Uns32 endianAddr;

    getArg(processor, object, 0, &port_nameAddr);
    getArg(processor, object, 1, &fileToLoadAddr);
    getArg(processor, object, 2, &endianAddr);
    ...
    if (!flashBase) {
        vmiMessage("F", PREFIX "_NP", "Failed to allocate FLASH ram for port %s at 0x%08x to 0x%08x (%u bytes)",
                                       portName, (Uns32)lo, (Uns32)hi, flashSize);
    }
    ...
    vmiMessage("I", PREFIX "_NP", "Allocated FLASH ram for port %s at 0x%08x to 0x%08x (%u bytes)",
                                   portName, (Uns32)lo, (Uns32)hi, flashSize);
...
static VMIOS_CONSTRUCTOR_FN(constructor)
{
    ...
    if (strcmp(procType, "pse") != 0) {
        vmiMessage("F", PREFIX "_BP", "Processor must be a PSE\n");
    }
    memEndian  endian    = vmirtGetProcessorDataEndian(processor);
    if(endian != ENDIAN_NATIVE) {
        vmiMessage("F", PREFIX "_BE", "Host processor must same endian as a PSE\n");
    }
...
vmiosAttr modelAttrs = {
    ...
    // -------------------   ----------- ------ -----------------
    .intercepts = {
        {"flashInit",         0,       True,   flashInit     },
        {"flashSetDiag",      0,       True,   flashSetDiag  },
        {0}
    }
};
