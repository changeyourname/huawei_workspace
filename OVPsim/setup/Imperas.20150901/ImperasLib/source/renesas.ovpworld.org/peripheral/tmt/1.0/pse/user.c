...
//
void interceptPlantModelDuty(Uns32 ccr0, Uns32 ccr1, Uns32 rate)
{
    bhmMessage("F", PREFIX, "Failed to intercept : %s : Please Ensure Plant Model loaded", __FUNCTION__);
}
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    if(bhmBoolAttribute("PlantModelPWMMode")) {
        plantModelEnabled = True;
        if (PSE_DIAG_HIGH) bhmMessage("I", PREFIX"_PE", "Plant Model PWM Mode Enabled");
    }
