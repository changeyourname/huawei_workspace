...
static Uns8 timerStack[STACKSIZE];
...
static void timerThread(void *user) {
    while (1) {
        ...
        if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "Timer Expired %f", bhmGetCurrentTime());

        //
        // OK, we have timed out, question is why ?
        //
        evaluateEvent();
        if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "Re-Start Timer(%f) %f",  microSecondDelay, bhmGetCurrentTime());
    }
}

static void stopTimer() {
    if (timerThreadHandle) {
        if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "Stop Timer %f", bhmGetCurrentTime());
...
static void startTimer(double delay) {
    ...
    if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "Start Timer(%f) %f",  microSecondDelay, bhmGetCurrentTime());
    timerThreadHandle = bhmCreateThread(timerThread, 0, "timer", &(timerStack[STACKSIZE]));
}
...
static Uns32 updateCntInfo (Uns16 CNT) {
    ...
    if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "updateCntInfo: CNT=0x%04x", cntInfo.value);
    //
    ...
    if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "updateCntInfo: nextCNT=0x%04x ticks=0x%04x", cntInfo.next, ticks);
...
static void genSignalsFromCCR() {
    ...
    if (cntInfo.next == CCR_Buffer[0]) {
     	if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "%s:%d", __FUNCTION__, __LINE__);
	...
	if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "%s:%d", __FUNCTION__, __LINE__);
	    ...
	    if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "%s_ADTRG0:%d", __FUNCTION__, __LINE__);
            posPulseNet(&TSADTRG0);
        }
        if(TMSP0_reg3_data.OPT3.bits.AT04) {
            if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "%s_ADTRG1:%d", __FUNCTION__, __LINE__);
...
void update_timer () {
    ...
    if (TMSP0_reg0_data.CTL1.bits.EEE) {
        ...
        if (BHM_DIAG_HIGH) bhmMessage("I", "EVCB", "PCLKS=%f ClockInterval=%fus", PCLKS, microSecCB);
    }

    if (CE) {
        if (BHM_DIAG_HIGH) bhmMessage("I", "EVCB", "Counter Enabled");
	    ...
	    if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "%s:%d", __FUNCTION__, __LINE__);
            ...
            if (BHM_DIAG_HIGH) bhmMessage("W", "EVCB", "Interval Timer Mode ticks=%d", ticks);
        ...
        case 1:
            ...
            //
            bufferReloadCCR();
            if (BHM_DIAG_HIGH) bhmMessage("W", "CTL1", "Untested MODE: External Event Count");
            break;
        case 2:
            //
            // External Trigger Pulse Output Mode
	    ...
	    if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "%s:%d", __FUNCTION__, __LINE__);
            ...
            bufferReloadCCR();
            ticks = updateCntInfo(0);
            if (BHM_DIAG_HIGH) bhmMessage("W", "EVCB", "Free-Running Mode ticks=%d", ticks);
            startTimer(ticksToTime(ticks));
            break;
        case 7:
            //
        ...
        //
        if (BHM_DIAG_HIGH) bhmMessage("I", "EVCB", "Counter Disabled");
...
void update_count () {
    ...
    if (BHM_DIAG_HIGH) bhmMessage("I", "TMS", "%s %d %d %f", __FUNCTION__, cntInfo.value, cnt, cntInfo.time);
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    //
    bhmIntegerAttribute("PCLK0", &PCLK[0]);
    bhmIntegerAttribute("PCLK1", &PCLK[1]);
    bhmIntegerAttribute("PCLK2", &PCLK[2]);
    bhmIntegerAttribute("PCLK3", &PCLK[3]);
    bhmIntegerAttribute("PCLK4", &PCLK[4]);
    bhmIntegerAttribute("PCLK5", &PCLK[5]);
    bhmIntegerAttribute("PCLK7", &PCLK[7]);
    bhmIntegerAttribute("PCLK9", &PCLK[9]);
