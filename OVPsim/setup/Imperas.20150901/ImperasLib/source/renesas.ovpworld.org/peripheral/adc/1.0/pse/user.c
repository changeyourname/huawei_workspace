...
static Uns8 idleStack[STACKSIZE];
...
PPM_NET_CB(P_INTADEN) {
    ...
    if(DIAG_HIGH)
        bhmMessage("I", PREFIX"_DMAEN", "Net Value %d", value);
    if (value) {
        DMAEnabled=True;
    } else {
        DMAEnabled=False;
...
PPM_REG_READ_CB(readADCR) {
    Int32 idx = (Int32) user;
    if(DIAG_HIGH)
        bhmMessage("I", PREFIX"_ADCR", "Channel Index %d", idx);
    return readSample((Uns32)addr, bytes, idx);
}
...
//        idleThreadHandle = bhmCreateThread(idleThread, 0, "idleThread", &(idleStack[STACKSIZE]));
    } else {
        //
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    for (ch=0; ch < ANIN; ch++) {
        ...
        if (bhmStringAttribute(stimAttrName, buffer, sizeof(buffer))) {
        ...
        if (bhmStringAttribute(plantAttrName, buffer, sizeof(buffer))) {
    ...
    // PCLK1
    //
    bhmIntegerAttribute("PCLK1", &PCLK1);

    //
    // sample
    //
    Uns32 s1;
    if (bhmIntegerAttribute("sample", &s1)) {
        sample = s1;
    }
