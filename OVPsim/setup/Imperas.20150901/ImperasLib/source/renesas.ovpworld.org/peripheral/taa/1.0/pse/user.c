...
static void timerThread(void *user) {
    while (1) {
        ...
        if (BHM_DIAG_HIGH) bhmMessage("I", "TAA", "Timer Expired %f", bhmGetCurrentTime());

        //
        // OK, we have timed out, question is why ?
        //
        evaluateEvent();
        if (BHM_DIAG_HIGH) bhmMessage("I", "TAA", "Re-Start Timer(%f) %f",  microSecondDelay, bhmGetCurrentTime());
    }
}

static void stopTimer() {
    if (timerThreadHandle) {
        if (BHM_DIAG_HIGH) bhmMessage("I", "TAA", "Stop Timer %f", bhmGetCurrentTime());
...
static void startTimer(double delay) {
    ...
    if (BHM_DIAG_HIGH) bhmMessage("I", "TAA", "Start Timer(%f) %f",  microSecondDelay, bhmGetCurrentTime());
...
static Uns32 updateCntInfo (Uns16 CNT) {
    ...
    if (BHM_DIAG_HIGH) bhmMessage("I", "TAA", "updateCntInfo: CNT=0x%04x", cntInfo.value);
    //
    ...
    if (BHM_DIAG_HIGH) bhmMessage("I", "TAA", "updateCntInfo: nextCNT=0x%04x", cntInfo.next);
...
//
void interceptPlantModelDutyRate(Uns32 cycle, Uns32 duty, Uns32 ticksPerSecond)
{
    bhmMessage("F", "DUTYRATE", "Failed to intercept : %s : Please Ensure Plant Model loaded", __FUNCTION__);
}
...
void update_timer () {
    ...
    if (TAAP0_reg0_data.CTL1.bits.EEE) {
        ...
        if (BHM_DIAG_HIGH) bhmMessage("W", "EVCB", "PCLKS=%f ClockInterval=%fus", PCLKS, microSecCB);
    }

    if (CE) {
        if (BHM_DIAG_HIGH) bhmMessage("W", "EVCB", "Counter Enabled");
...
//            }
        ...
        if (BHM_DIAG_HIGH) bhmMessage("W", "EVCB", "Counter Disabled");
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    plantModelEnabled = bhmBoolAttribute("PlantModelPWM");
