...
static void timerThread(void *user) {
    while (1) {
        ...
        if (BHM_DIAG_HIGH) bhmMessage("I", "BRG", "Timer Expired %f", bhmGetCurrentTime());
...
static void stopTimer(void) {
    if (timerThreadHandle) {
        if (BHM_DIAG_HIGH) bhmMessage("I", "BRG", "Stop Timer %f", bhmGetCurrentTime());
...
static void startTimer(double delay) {
    stopTimer();
    threadDelay = delay;
    if (BHM_DIAG_HIGH) bhmMessage("I", "BRG", "Start Timer(%f) %f",  delay, bhmGetCurrentTime());
