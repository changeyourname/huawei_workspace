...
static Uns8 rxStack[STACKSIZE];
static Uns8 txStack[STACKSIZE];
...
static Int32 readInput(Uns8 *buffer, Uns32 length) {
    Int32 bytes = 0;
    if (channel >= 0) {
        bytes = bhmSerRead(channel, buffer, length);
    }
    return bytes;
}
...
static Uns32 writeOutput(Uns8 *buffer, Uns32 length)
{
    ...
    if (channel >= 0) {
        ...
        if (rc != length) {
            if (!writeError) {
                bhmMessage("W", "SWE", "Serial channel write error. Channel closed?");
                writeError = True;
            }
        } else {
            writeError = False;
...
PPM_REG_WRITE_CB(writeCTL0) {
    ...
    *(Uns8*)user = data;
    ...
    if (UARTP0_reg0_data.CTL0.bits.RXE != oldRXE) {
        if (UARTP0_reg0_data.CTL0.bits.RXE) {
            rxThreadHandle = bhmCreateThread(rxThread, NULL, "RX", &(rxStack[STACKSIZE]));
        } else {
    ...
    if (UARTP0_reg0_data.CTL0.bits.TXE != oldTXE) {
        if (UARTP0_reg0_data.CTL0.bits.TXE) {
            txThreadHandle = bhmCreateThread(txThread, NULL, "TX", &(txStack[STACKSIZE]));
        } else {
