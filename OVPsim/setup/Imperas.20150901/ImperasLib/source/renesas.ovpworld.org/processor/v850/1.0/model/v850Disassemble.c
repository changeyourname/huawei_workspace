...
const char *formats[V850_IT_LAST+1] = {
    ...
    //
    [V850_IT_LDSR_F09RS1]    = EMITOP " " EMITR1  ", " EMITSR2,
    ...
    //
    [V850_IT_STSR_F09SR]     = EMITOP " " EMITSR1 ", " EMITR2,
    ...
    //
    [V850_IT_DISPOSE_F13IL1] = EMITOP " " EMITUI5  ", " EMITL12,

    //
    // Instruction Format F13il2 Insns
    //
    [V850_IT_DISPOSE_F13IL2] = EMITOP " " EMITUI5  ", " EMITL12 ", " EMITR2,
...
static void EMITL12_FN(char **result, v850InstructionInfoP info) {
    char tmp[32];
    const char *fmt = "L={0x%03x}";
    sprintf(tmp, fmt, info->list12);
    putString(result, tmp);
}
//
...
static void EMITSDR_FN(char **result, v850InstructionInfoP info) {
    char tmp[32];
    const char *fmt = "0x%x";
    sprintf(tmp, fmt, info->sdisp + info->thisPC);
    putString(result, tmp);
}
//
...
// Function to format the info field #reg1#
//
static void EMITSR1_FN(char **result, v850InstructionInfoP info) {
    char tmp[32];
    const char *fmt = (SPR[info->reg1] ? SPR[info->reg1] : "sr%d");
    sprintf(tmp, fmt, info->reg1);
    putString(result, tmp);
}
//
// Function to format the info field #reg2#
//
static void EMITSR2_FN(char **result, v850InstructionInfoP info) {
    char tmp[32];
    const char *fmt = (SPR[info->reg2] ? SPR[info->reg2] : "sr%d");
    sprintf(tmp, fmt, info->reg2);
    putString(result, tmp);
}
//
...
static void disassembleFormat(
    ...
    while((ch=*format++)) {
         switch(ch) {
             ...
             case EMITSR1_CASE:
                 EMITSR1_FN(result, info);
                 break;

             case EMITSR2_CASE:
                 EMITSR2_FN(result, info);
                 break;
