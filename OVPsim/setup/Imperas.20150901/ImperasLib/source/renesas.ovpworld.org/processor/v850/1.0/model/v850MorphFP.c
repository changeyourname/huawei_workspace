...
static void vmic_emitFPConvE1(v850P v850, Uns32 rB, Uns32 rD) {
    ...
    if        (v850->FLG_FPF & FPFLAGS_P) {
        ...
        if (IS_V850_FP32_NAN(arg)) {
            v850->gpr[rD] = V850_FP32_MINUS_ZERO;
            v850->FLG_FPF |= FPFLAGS_D;

        } else if (IS_V850_FP32_INF(arg)) {
            if (V850_FP32_SIGN(arg)) {
                v850->gpr[rD] = V850_INT32_MIN;
            } else {
                v850->gpr[rD] = V850_INT32_MAX;
            }
            v850->FLG_FPOV = 1;
            v850->FLG_FPF |= FPFLAGS_D;
...
static void emitFPConvE1(v850MorphStateP state, vmiReg rB, Uns32 rBi, vmiFType tB, vmiReg rD, Uns32 rDi, vmiFType tD, vmiFPRC rc) {
    ...
    vmimtMoveRC(V850_FLG_FPOV_BITS, V850_FLG_FPOV_WR, 0);
...
static void emitFPFlagsE1(v850MorphStateP state, vmiReg rD, Bool Z, Bool S) {
    if (Z) {
        ...
        //
        vmimtTestRC(32, vmi_COND_Z, rD, 0x7fffffff, V850_FLG_FPZ_WR);
    }
    if (S) {
        //
        // If -ve, set the S flag
        //
        vmimtTestRC(32, vmi_COND_NZ, rD, 0x80000000, V850_FLG_FPS_WR);
    }
}
...
static void CMOVF(Uns8 fff, vmiReg r1, vmiReg r2, vmiReg r3, Uns8 size) {
    //
    // Test V850_FPSR_CC_WR(fff)
    // if (cc==1) r3 <- r1
    // else       r3 <- r2
    //
    vmiLabelP E1 = vmimtNewLabel();
    vmiLabelP F1 = vmimtNewLabel();
    vmimtCompareRCJumpLabel(8, vmi_COND_EQ, V850_FPSR_CC_WR(fff), 0, F1);
...
static void cmpf(v850MorphStateP state, vmiFType ftype) {
    ...
    vmimtFCompareRR(ftype, V850_FLG_FCMP_WR, gpr_reg1, gpr_reg2, V850_FLG_FPF_WR, allowQNaN, 0);
    ...
    // FPSR_CC[fff] = (V850_FLG_FCMP_WR & cond) ? 1 : 0;
    // update field of FPSR/CC Temporary
    vmimtTestRC(8, vmi_COND_NZ, V850_FLG_FCMP_RD, cond, V850_FPSR_CC_WR(fff));
}
...
V850_MORPH_FN(morphMAXF_S_FIS) {
    ...
    //
    // Do temporary addition of ZERO, to flush input args to 0 when denormal
    // ??? if the arg is an snan, it is converted to qnan, and then no flag generation
    // ??? in the max(a,b)
    //
//    vmimtMoveRC(V850_GPR_BITS, V850_TMP_WR(0), V850_FP32_PLUS_ZERO);
//    vmimtFBinopRRR(vmi_FT_32_IEEE_754, vmi_FADD, gpr_reg1, V850_TMP_WR(0), gpr_reg1, VMI_NOREG, 0);
//    vmimtMoveRC(V850_GPR_BITS, V850_TMP_WR(1), V850_FP32_PLUS_ZERO);
//    vmimtFBinopRRR(vmi_FT_32_IEEE_754, vmi_FADD, gpr_reg2, V850_TMP_WR(1), gpr_reg2, VMI_NOREG, 0);
//    emitFPBinopE2(state, vmi_FT_32_IEEE_754, gpr_reg1, gpr_reg2, gpr_reg3, vmi_FMAX);
...
V850_MORPH_FN(morphTRFF_FIV) {
    ...
    // Supported Architecture=V850E1_SP_FPU
    //
    //    FLG_CO = (FLG_FPF_STICKY != 0)
    //    FLG_OV = FLG_FPOV
    //    FLG_SI = FLG_FPS
    //    FLG_ZR = FLG_FPZ
    vmimtCondMoveRCC(V850_FLG_CO_BITS, V850_FLG_FPF_STICKY_RD, False, V850_FLG_CO_WR, 0, 1);
    vmimtMoveRR(V850_FLG_OV_BITS, V850_FLG_OV_WR, V850_FLG_FPOV_RD);
    vmimtMoveRR(V850_FLG_SI_BITS, V850_FLG_SI_WR, V850_FLG_FPS_RD);
    vmimtMoveRR(V850_FLG_ZR_BITS, V850_FLG_ZR_WR, V850_FLG_FPZ_RD);
}

static void vmic_LDFF_FIVR2A(v850P v850, Uns32 reg2) {
    v850PackEFG(v850);
    MTSREG(EFG, reg2);
    v850UnPackEFG(v850);
}
V850_MORPH_FN(morphLDFF_FIVR2A) {
    ...
    vmimtArgProcessor();
    vmimtArgUns32(reg2);
    vmimtCall((vmiCallFn)vmic_LDFF_FIVR2A);
}

static void vmic_LDFC_FIVR2B(v850P v850, Uns32 reg2) {
    MTSREG(ECT, reg2);
}
V850_MORPH_FN(morphLDFC_FIVR2B) {
    ...
    vmimtArgProcessor();
    vmimtArgUns32(reg2);
    vmimtCall((vmiCallFn)vmic_LDFC_FIVR2B);
}

static void vmic_STFF_FIVR3A(v850P v850,Uns32 reg3) {
    v850PackEFG(v850);
    MFSREG(reg3, EFG);
}
V850_MORPH_FN(morphSTFF_FIVR3A) {
    ...
    vmimtArgProcessor();
    vmimtArgUns32(reg3);
    vmimtCall((vmiCallFn)vmic_STFF_FIVR3A);
}
...
V850_MORPH_FN(morphSTFC_FIVR3B) {
    ...
    Uns8 reg3 = state->info.reg3;
    vmiReg gpr_reg3 = V850_GPR_WR(reg3);
    vmimtMoveRR(V850_GPR_BITS, gpr_reg3, V850_SPR_ECT_RD);
}
...
V850_MORPH_FN(morphTRFSR_FIFFF) {
    ...
    //
    Uns8 fff = state->info.fff;
    vmimtUnopRR(8, vmi_MOV, V850_FLG_ZR_WR, V850_FPSR_CC_RD(fff), 0);
}
...
void v850UpdateFPControlWord(v850P v850) {
    ...
    if (v850->configInfo.arch & ISA_E1_FP) {
        ...
        fpcw.IM = !v850->SPR_FPSR.bits.XE_V;
        fpcw.DM = 1;
        fpcw.ZM = !v850->SPR_FPSR.bits.XE_Z;
        fpcw.OM = !v850->SPR_FPSR.bits.XE_O;
        fpcw.UM = !v850->SPR_FPSR.bits.XE_U;
        fpcw.PM = !v850->SPR_FPSR.bits.XE_I;

        //
        // Rounding Control
        //
        fpcw.RC  = v850->SPR_FPSR.bits.RM; // Always vmi_FPR_NEAREST;
        fpcw.FZ  = v850->SPR_FPSR.bits.FS; // Always True
