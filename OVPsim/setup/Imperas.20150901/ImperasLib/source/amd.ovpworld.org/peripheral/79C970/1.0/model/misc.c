...
{
    DEBUG_SLIRP("insque(%s, %s)\n", slirp_ptr_anon(a), slirp_ptr_anon(b));
    VMI_ASSERT(a!=b, "List loop formed");
...
void slirp_connection_info(Slirp *slirp)
{
    ...
    vmiPrintf( "  Protocol[State]    FD  Source Address  Port   "
    ...
    for (so = slirp->tcb.so_next; so != &slirp->tcb; so = so->so_next) {
        ...
        memset(&buf[n], ' ', 19 - n);
        buf[19] = 0;
        vmiPrintf( "%s %3d %15s %5d ", buf, so->s,
                       src.sin_addr.s_addr ? inet_ntoa(src.sin_addr) : "*",
                       ntohs(src.sin_port));
        vmiPrintf( "%15s %5d %5d %5d\n",
...
{
    int ret = recOpenSocket(domain, type, protocol);
#if (IMPERAS_LINUX==1)
    if (ret >= 0) {
        int f = fcntl(ret, F_GETFD);
        fcntl(ret, F_SETFD, f | FD_CLOEXEC);
    }
#endif
    return ret;
}
...
Uns64 getClockMS(void) {

    struct timeval tt;
    gettimeofday(&tt, 0);

    Uns64 t =  (u_int)tt.tv_sec   * (Uns64)1000;
          t += (u_int)tt.tv_usec  / (Uns64)1000;

    return t;
}

Uns64 getClockNS(void) {

    struct timeval tt;
    gettimeofday(&tt, 0);

    Uns64 t =  (u_int)tt.tv_sec   * (Uns64)1000 * (Uns64)1000 * (Uns64)1000;
          t += (u_int)tt.tv_usec  * (Uns64)1000;
