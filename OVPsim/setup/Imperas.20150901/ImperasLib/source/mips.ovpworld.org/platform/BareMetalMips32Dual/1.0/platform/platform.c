...
static void createPlatform() {
    ...
    const char *mipsSemihost = icmGetVlnvString(NULL, "mips.ovpworld.org", "semihosting", "mips32SDE", "1.0", "model");

    icmAttrListP icmAttr1 = icmNewAttrList();
    icmAddStringAttr(icmAttr1, "endian",        "big");
    icmAddStringAttr(icmAttr1, "variant",       "4Km");		// default variant
    ...
    icmAddStringAttr(icmAttr2, "variant",       "4Km");     // default variant
...
static void addPlatformDocumentation() {
    ...
    icmDocNodeP Root1_node = icmDocSectionAdd(0, "Root");
    {
        ...
        icmDocTextAdd(doc_12_node, "Bare Metal Platform for a MIPS32 Processor (default 4Km)."
    ...
    "The ICM platform can be passed any application compiled to a MIPS elf format as the first argument. The"
