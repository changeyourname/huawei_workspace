...
#define INFO if (BHM_DIAG_HIGH) {bhmMessage("I", "US", "val=0x%08x", data);}
...
void write_irq(Uns32 value) {
    if (state.irq != value) {
        state.irq = value;
        if (BHM_DIAG_HIGH) bhmMessage("I", "US", "Setting Interrupt IRQ=%d\n", state.irq);
...
static Uns32 writeOutput(Uns8 *buffer, Uns32 length)
{
    ...
    if (state.channel >= 0) {
        ...
        if (rc != length) {
            if (!state.writeError) {
                bhmMessage("W", "US", "Channel write error. Channel closed?");
