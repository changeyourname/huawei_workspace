...
void write_irq0(Uns32 value) {
    if (state.irq0 != value) {
        state.irq0 = value;
        if (BHM_DIAG_HIGH) bhmMessage("I", "TC", "Setting Interrupt IRQ0=%d\n", state.irq0);
...
void write_irq1(Uns32 value) {
    if (state.irq1 != value) {
        state.irq1 = value;
        if (BHM_DIAG_HIGH) bhmMessage("I", "TC", "Setting Interrupt IRQ1=%d\n", state.irq1);
...
void write_irq2(Uns32 value) {
    if (state.irq2 != value) {
        state.irq2 = value;
        if (BHM_DIAG_HIGH) bhmMessage("I", "TC", "Setting Interrupt IRQ2=%d\n", state.irq2);
...
static void timerThread(void *user) {
    while (1) {
        ...
        if (BHM_DIAG_HIGH) bhmMessage("I", "TC", "Start Timer Expired");
        write_irq1(1);
        write_irq1(0);
    }
...
static void startTimer(Uns64 delay) {
    stopTimer();
    threadDelay = delay;
    if (BHM_DIAG_HIGH) bhmMessage("I", "TC", "Start Timer for %lldus", delay);
