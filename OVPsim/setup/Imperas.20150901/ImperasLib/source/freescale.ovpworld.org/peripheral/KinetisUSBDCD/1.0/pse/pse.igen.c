...
static PPM_WRITE_CB(bport1_ab_CLOCK_maskWrite) {
    bport1_ab_data.CLOCK.value = (bport1_ab_data.CLOCK.value & 0xfffff002) | (data & 0xffd);
}

static PPM_WRITE_CB(bport1_ab_CONTROL_maskWrite) {
    bport1_ab_data.CONTROL.value = (bport1_ab_data.CONTROL.value & 0xfcfefffe) | (data & 0x3010001);
}

static PPM_WRITE_CB(bport1_ab_TIMER0_maskWrite) {
    bport1_ab_data.TIMER0.value = (bport1_ab_data.TIMER0.value & 0xfe00ffff) | (data & 0x1ff0000);
}

static PPM_WRITE_CB(bport1_ab_TIMER1_maskWrite) {
    bport1_ab_data.TIMER1.value = (bport1_ab_data.TIMER1.value & 0xfc00fc00) | (data & 0x3ff03ff);
}

static PPM_WRITE_CB(bport1_ab_TIMER2_maskWrite) {
    bport1_ab_data.TIMER2.value = (bport1_ab_data.TIMER2.value & 0xfc00fff0) | (data & 0x3ff000f);
}
...
PPM_NET_CB(reset_Reset) {
    if(value != 0 ) {
        bport1_ab_data.TIMER2.value = 0x28000a;
        bport1_ab_data.TIMER1.value = 0xa0028;
        bport1_ab_data.TIMER0.value = 0x100000;
        bport1_ab_data.STATUS.value = 0x0;
        bport1_ab_data.CLOCK.value = 0xc1;
        bport1_ab_data.CONTROL.value = 0x10000;
    }
}
