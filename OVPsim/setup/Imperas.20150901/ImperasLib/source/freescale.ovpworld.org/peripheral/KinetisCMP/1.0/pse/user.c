...
struct {
    ...
    ppmNetValue  currDacOutputValue;
    ppmNetValue  currCoutSocValue;
    ppmNetValue  currInterruptValue;
    ppmNetValue  currDmaReqValue;
    ppmNetValue  currCmpOPadValue;
...
//    Uns32 lastCOUTValue = bport1_ab_data.SCR.bits.COUT;
    if (!bport1_ab_data.CR1.bits.EN) {
        ...
        ppmNetValue minusValue = state.lastInputValue[bport1_ab_data.MUXCR.bits.MSEL];
        ppmNetValue plusValue = state.lastInputValue[bport1_ab_data.MUXCR.bits.PSEL];
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    bhmIntegerAttribute("cmpNumber", &(cmp_in.chNum));
    ...
    if (bhmStringAttribute(stimAttrName, state.stimFile, sizeof(state.stimFile)) &&
    ...
    // CR0
    if (bhmIntegerAttribute("configure_cr0", &configBits)) {
    ...
    // CR1
    if (bhmIntegerAttribute("configure_cr1", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // FPR
    if (bhmIntegerAttribute("configure_fpr", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // SCR
    if (bhmIntegerAttribute("configure_scr", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // DACCR
    if (bhmIntegerAttribute("configure_daccr", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // MUXCR
    if (bhmIntegerAttribute("configure_muxcr", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // controllerThreadHandle = bhmCreateThread(controllerThread, 0, "controllerThread", &(controllerStack[STACKSIZE]));
    char threadName[32];
    if (cmp_in.fp) {
        ...
        thState.thread = bhmCreateThread(controllerThread, (void *)0, threadName, &thState.stack[THREAD_STACK]);
