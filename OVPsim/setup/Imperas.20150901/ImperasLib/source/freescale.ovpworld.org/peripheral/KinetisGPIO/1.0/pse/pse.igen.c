...
static void installNetPorts(void) {
    ...
    handles.Input = ppmOpenNetPort("Input");
    if (handles.Input) {
        ppmInstallNetCallback(handles.Input, inputCB, (void*)0);
    }

// To write to this net, use ppmWriteNet(handles.Output, value);

    handles.Output = ppmOpenNetPort("Output");

// To write to this net, use ppmWriteNet(handles.OutputMask, value);

    handles.OutputMask = ppmOpenNetPort("OutputMask");

}

PPM_NET_CB(reset_Reset) {
    if(value != 0 ) {
        bport1_ab_data.PDDR.value = 0x0;
