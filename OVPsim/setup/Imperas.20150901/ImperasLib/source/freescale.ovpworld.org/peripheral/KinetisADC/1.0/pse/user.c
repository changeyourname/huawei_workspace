    ...
    if (state.currAdInIdValue != value) {
        ...
        if (handles.AdInId) {
            if (state.isRunningUnitTest) {
                bhmMessage("I", "KinetisADC", "        setting adinid = %d", state.currAdInIdValue);
            }
...
static void updateInterruptOutput(Uns32 value) {
    ...
    if (state.currInterruptValue != value) {
        ...
        if (handles.Interrupt) {
            ...
            if (state.isRunningUnitTest) {
                bhmMessage("I", "KinetisADC", "        setting interrupt = %d", state.currInterruptValue);
            }
...
static void updateDmaReqOutput(Uns32 value) {
    ...
    if (state.currDmaReqValue != value) {
        ...
        if (handles.DmaReq) {
            ...
            if (state.isRunningUnitTest) {
                bhmMessage("I", "KinetisADC", "        setting dma request = %d", state.currDmaReqValue);
            }
...
static Uns8 controllerStack[STACKSIZE];
...
static Uns8 stimulusTriggerStack[STACKSIZE];
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    for (ch=0; ch < NUM_CHANNELS; ch++) {
        ...
        if (bhmStringAttribute(stimAttrName, state.stimFile[ch], sizeof(state.stimFile[ch]))) {
    ...
    if (bhmIntegerAttribute("bus_clock_freq", &configBits)) {
        if (0 < configBits) {
    ...
    // SC1A
    if (bhmIntegerAttribute("configure_sc1a", &configBits)) {
    ...
    // SC1B
    if (bhmIntegerAttribute("configure_sc1b", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // CFG1
    if (bhmIntegerAttribute("configure_cfg1", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // CFG2
    if (bhmIntegerAttribute("configure_cfg2", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // CV1
    if (bhmIntegerAttribute("configure_cv1", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // CV2
    if (bhmIntegerAttribute("configure_cv2", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // SC2
    if (bhmIntegerAttribute("configure_sc2", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // SC3
    if (bhmIntegerAttribute("configure_sc3", &configBits)) {
        state.isRunningUnitTest = True;
    ...
    // Vrefh
    if (bhmIntegerAttribute("input_vrefh", &inputBits)) {
        state.isRunningUnitTest = True;
    ...
    // Vrefl
    if (bhmIntegerAttribute("input_vrefl", &inputBits)) {
        state.isRunningUnitTest = True;
    ...
    if (adc_in[0].fp) {
        if ((0 == bport1_ab_data.CFG1.bits.ADICLK) || (1 == bport1_ab_data.CFG1.bits.ADICLK))
        {
            controllerThreadHandle = bhmCreateThread(syncControllerThread, 0, "syncControllerThread", &(controllerStack[STACKSIZE]));
            stimulusTriggerThreadHandle = bhmCreateThread(stimulusTriggerThread, 0, "stimulsTriggerThread", &(stimulusTriggerStack[STACKSIZE]));
        } else {
            controllerThreadHandle = bhmCreateThread(asyncControllerThread, 0, "asyncControllerThread", &(controllerStack[STACKSIZE]));
        }
    } else {
