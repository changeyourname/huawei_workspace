...
Uns32 baudDelay() {
    ...
    if (sbr > 0) {
        ...
        if (baud != 0) {
            ...
            if (DIAG_MED && baudDelay != oldDelay) {
                bhmMessage(
                    "I", PREFIX "_BRC",
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    bhmIntegerAttribute("fifoSize",       &state.fifoSize);
    bhmIntegerAttribute("moduleClkFreq",  &state.moduleClkFreq);
    ...
    Uns32 portnum;
    // If DirectRead and DirectWrite nets are connected then use direct mode
    state.directMode = handles.DirectRead && handles.DirectWrite && !bhmBoolAttribute("console") && !bhmIntegerAttribute("portnum", &portnum);
    ...
    if (state.directMode) {
        ...
        bhmCreateThread(directWriteThread, NULL, "directWriteThread", &threadStack[TOP]);
        ...
        openChannel();
        bhmCreateThread(channelReadThread, NULL, "channelReadThread", &threadStack[TOP]);
