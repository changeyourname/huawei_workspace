...
void doReset() {
    ...
    macReg.IA0_1.value = 0x1200;
    macReg.IA2_3.value = 0x5634;
    macReg.IA4_5.value = 0x9a78;
...
void send_tx_packet(Uns8 packet_num)
{
    ...
    if (DIAG_3) {
        bhmMessage("I", PREFIX, "send_tx_packet: packet num=%d, len=%d", packet_num, length);
    }
   ...
   if (!result) {
       // todo
        ...
        //macReg.INTERRUPT.bits.TX_INT = 1;
...
static void rxPollThread(void *user)
{
    ...
    while(1) {
        ...
        if (state.irqAssertCnt > 0) {
            state.irqAssertCnt--;
            if (state.irqAssertCnt == 0) {
                //ppmWriteNet(handles.irq, 0);
            }
        }

        if (!macReg.RCR.bits.RXEN)
            continue;

        Int32 readBytes = bhmEthernetReadFrame(state.ethHandle, state.rx_packet_buf, sizeof(state.rx_packet_buf), timeMS, 1);
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    state.ethHandle = bhmEthernetOpenAuto(); //(2,0,"etherinlog.dat","etherlog.dat"); //OpenAuto();
    doReset();
