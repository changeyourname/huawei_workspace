...
//        return state.phy_int_mask;
    ...
    if (DIAG_3) {
        bhmMessage("I", PREFIX, "Read PHY register index=%u, value=0x%04x", index, (unsigned)value);
    }
}
...
static void do_phy_write()
{
    ...
    switch (index) {
     case 0: /* Basic Control */
          phyReg.BasicControl.value = value;
//        if (val & 0x8000) {
//            phy_reset(s);
//            break;
//        }
//        state.phy_control = val & 0x7980;
//        /* Complete autonegotiation imediately.  */
//        if (val & 0x1000) {
//            state.phy_status |= 0x0020;
//        }
         break;
      case 4: /* Auto-neg advertisment */
...
static void rxPollThread(void *user)
{
    ...
    while(1) {
        ...
        Int32 readBytes = bhmEthernetReadFrameN(state.ethHandle, state.rx_packet_buf, sizeof(state.rx_packet_buf), timeMS, 1);

        if (readBytes > 0) {
            // Add a pseudo FCS if needed as a partial workaround for missing MAC functionality
            readBytes += 4;

            pushRxFrame(state.rx_packet_buf, readBytes);
        }
    }
#endif
#if(0)
    while(1) {
        Int32 readBytes = bhmEthernetReadFrameB(state.ethHandle, state.rx_packet_buf, sizeof(state.rx_packet_buf), 0, 1);
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    doReset();
    if (state.ethHandle != (Uns32)(-1))
        phyReg.BasicStatus.bits.AutoNegotiationComplete = phyReg.BasicStatus.bits.LinkStatus = 1U;

    bhmCreateThread(rxPollThread,  NULL,  "rxPollThread", &state.stack[THREAD_STACK]);
}
