...
static void cmdParserAddUserArgs(icmCLPP parser) {
    ...
    icmCLParserAdd(parser, "linuxcmd", 0 , 0, "userPlatformConfig", ICM_AT_STRINGVAL    , &options.linuxcmd, "Linux command line (default: 'mem=2G@0x80000000 raid=noautodetect console=ttyAMA0,38400n8 devtmpfs.mount=0", 0x0, 0, 0);
    ...
    icmCLParserAdd(parser, "android" , 0 , 0, "userPlatformConfig", ICM_AT_BOOLVAL      , &options.android, "Enable android specific configuration options", 0x0, 0, 0);
    icmCLParserAdd(parser, "sdimage" , 0 , 0, "userPlatformConfig", ICM_AT_STRINGVAL    , &options.sdimage, "File containing SD Card image to load on MultiMedia Card Interace mmc1", 0x0, 0, 0);
}
