...
#define SMARTLOADER "HeteroAlteraCycloneV_HPS_CycloneIII_3c120/smartLoader/"
#define UART0       "HeteroAlteraCycloneV_HPS_CycloneIII_3c120/uart0/"
#define UART1       "HeteroAlteraCycloneV_HPS_CycloneIII_3c120/uart1/"
...
int main(int argc, const char *argv[]) {
    ...
    icmCLPP parser = icmCLParser("HeteroAlteraCycloneV_HPS_CycloneIII_3c120", ICM_AC_ALL);
...
//
    ...
    if (options.BOOT) {
        // Load the boot file
        icmLoadBus(handles.smbus_HPS_b, options.BOOT, ICM_LOAD_VERBOSE, True);
    }

    if (options.NIOSBOOT) {
        // Load the boot file
        icmLoadProcessorMemory(handles.cpu_Nios_II_c, options.NIOSBOOT, (ICM_LOAD_VERBOSE | ICM_SET_START), 0 , 0);
    }

    // Setup application on each leaf processor
    icmIterAllDescendants(handles.cpu_A9MPx2_c, setupApplication, NULL);
