...
static Uns32 mapArmFlags(Uns32 aFlags) {
    ...
    // ARM Angel gives flags
    // 0x01 - binary
    // 0x02 - rdwr
    // 0x04 - creat/trunc
    // 0x08 - append

    // if (aFlags & 0x0001) hostFlags |= O_BINARY;
    if (aFlags & 0x0002) hostFlags |= VMIOS_O_RDWR;
    if (aFlags & 0x0004) hostFlags |= VMIOS_O_WRONLY | VMIOS_O_CREAT | VMIOS_O_TRUNC;
    if (aFlags & 0x0008) hostFlags |= VMIOS_O_WRONLY | VMIOS_O_CREAT | VMIOS_O_APPEND;
...
static VMIOS_INTERCEPT_FN(armAngelSyncTrap) {
    ...
    switch (trapCode) {
        ...
        {
            vmiPrintf("%s", vmirtGetString(domain, trapArg));
            break;
        }
        ...
        {
            Uns32 reasonCode  = readNthParam(processor, object, trapArg, 0);
...
static Uns32 getNumCores(vmiProcessorP processor) {
    ...
    Uns32         count = 0;
    vmiProcessorP this = vmirtGetSMPParent(processor);

    if (this) {
        // SMP
        for(
            this=vmirtGetSMPChild(this);
            this;
            this=vmirtGetSMPNextSibling(this)
        ) {
            count++;
        }
    } else {
