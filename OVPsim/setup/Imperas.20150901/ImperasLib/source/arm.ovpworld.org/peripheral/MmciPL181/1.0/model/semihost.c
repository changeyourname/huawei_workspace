...
//
static VMIOS_INTERCEPT_FN(initNative) {

    if(DIAG_SEMIHOST_HIGH)
        vmiMessage("I", PREFIX, "%s", __FUNCTION__);

    Uns32 baseRegStruct;
    Uns32 fifoBase, fifoSize;
    Uns32 driveNameAddress, driveNameLen, delta;

    getArg(processor, object, 0, &baseRegStruct);
    getArg(processor, object, 1, &fifoBase);
    getArg(processor, object, 2, &fifoSize);
    getArg(processor, object, 3, &driveNameAddress);
    getArg(processor, object, 4, &driveNameLen);
    getArg(processor, object, 5, &delta);
    getArg(processor, object, 6, &object->pse.irq0);
    getArg(processor, object, 7, &object->pse.irq1);
    getArg(processor, object, 8, &object->pse.cardin);
    getArg(processor, object, 9, &object->pse.wprot);
    ...
    //
    if(DIAG_SEMIHOST_LOW)
        vmiMessage("I", PREFIX, "%s: Map Register Structure: Base 0x%08x", name, baseRegStruct);

    bport1_ab_dataTP bportRegs = &object->bportRegs;
    Uns32 regTop               = baseRegStruct+sizeof(bport1_ab_dataT)-1;
    ...
    if(DIAG_SEMIHOST_LOW)
        vmiMessage("I", PREFIX, "%s: SD Drive Init: %s, Delta %d, Base 0x%08x", name, sdDrive, delta, driveNameAddress);
    ...
    //
    if(DIAG_SEMIHOST_LOW)
        vmiMessage("I", PREFIX, "%s: FIFO Base 0x%0x Size 0x%0x", name, fifoBase, fifoSize);
...
static VMIOS_INTERCEPT_FN(startSDNative) {
    object->card = sd_init(processor, object->pse, object->bdrv ? object->bdrv : NULL, 0, object->diag);
}

static VMIOS_INTERCEPT_FN(setNativeDiagnostics) {

    if(DIAG_SEMIHOST_HIGH)
        vmiMessage("I", PREFIX, "%s", __FUNCTION__);

    Uns32 diagAddr, currentDiag;

    getArg(processor, object, 0, &diagAddr);
    getArg(processor, object, 1, &currentDiag);
...
static VMIOS_INTERCEPT_FN(sendCommandNative) {
...
static VMIOS_INTERCEPT_FN(writeDCNative) {
...
vmiosAttr modelAttrs = {
    ...
    // -------------------     ----------- ------  -----------------
    .intercepts = {
        {"initNative",                  0, True,  initNative },
        {"setNativeDiagnostics",        0, True,  setNativeDiagnostics },
        {"startSDNative",               0, True,  startSDNative },
        {"sendCommandNative",           0, True,  sendCommandNative },
        {"writeDCNative",               0, True,  writeDCNative },
        {0}
    }
};
