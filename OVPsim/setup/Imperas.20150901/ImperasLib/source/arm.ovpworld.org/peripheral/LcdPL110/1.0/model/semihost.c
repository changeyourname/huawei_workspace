    ...
    if (object->srcWidth) { // in case called too early
        ...
        if(PL110_DEBUG && object->verbose) {
            vmiMessage("I", PREFIX "_DES",
                "Desensitize wr:%d " FMT_Ax "-" FMT_Ax " row:%d",
                offset, lo, hi, row
            );
        }
...
static void putData(
    ...
    getArg(processor, object, 0, &address);
...
{
    Uns32   titleAddr, height, width;
    char    title[32] = "LCD PL110";
    object->pseDomain    = vmirtGetProcessorDataDomain(processor);


    getArg(processor, object, 0, &titleAddr);
    // fetch the Windows's title text.
    vmirtReadNByteDomain(object->pseDomain, titleAddr, &title, sizeof(title), 0, True);

    getArg(processor, object, 1, &height);
    getArg(processor, object, 2, &width);
    getArg(processor, object, 3, &object->bpp);
    getArg(processor, object, 4, &object->paletteSize);
    getArg(processor, object, 5, &object->paletteBase);
    getArg(processor, object, 6, &object->verbose);

    getArg(processor, object, 7, &object->artifactOffset);

    getArg(processor, object, 8, &object->packedPixels);
...
//
static VMIOS_INTERCEPT_FN(setVideoBase)
{
    Uns32       base;
...
//
static VMIOS_INTERCEPT_FN(updateFrame)
{
    // control register is passed for each frame update
    if (object->sdlDisplay && sdl_needs_update(object->sdlDisplay)) {
        Uns32 update;
        getArg(processor, object, 0, &object->controlReg);
        getArg(processor, object, 1, &update);
...
static VMIOS_INTERCEPT_FN(gp_resize) {
    Uns32 width, height, bpp;
    getArg(processor, object, 0, &width);
    getArg(processor, object, 1, &height);
    getArg(processor, object, 2, &bpp);
...
vmiosAttr modelAttrs = {
    ...
    .intercepts = {
        ...
        {"setVideoBase",            0,       True,  setVideoBase           },
        {"updateFrame",             0,       True,  updateFrame            },
        {"gp_resize",               0,       True,  gp_resize              },
