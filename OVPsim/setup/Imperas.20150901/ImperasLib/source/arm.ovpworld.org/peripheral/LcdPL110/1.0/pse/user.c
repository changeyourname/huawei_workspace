...
//
void setVideoBase(Uns32 baseAddress) {
    FAILED_TO_INTERCEPT;
}
...
//
void updateFrame(Uns32 controlReg, Uns32 update) {
    FAILED_TO_INTERCEPT;
}
...
//
void gp_resize(Uns32 width, Uns32 height, Uns32 bpp) {
    FAILED_TO_INTERCEPT;
}
...
Uns32 ImperasTestModeTimeout(double * timeout)
{
    FAILED_TO_INTERCEPT;
    return (Uns32)NULL;
}
...
PPM_CONSTRUCTOR_CB(constructor) {
    ...
    bhmIntegerAttribute(SCAN_DELAY_ATTRIBUTE,  &period);
    bhmIntegerAttribute(BUS_OFFSET_ATTRIBUTE, &busOffset);

    refreshPeriod = period;

    char title[32] = "PL110 LCD";
    bhmStringAttribute("title", title, sizeof(title));

    char resolution[16] = "vga";
    if (bhmStringAttribute("resolution", resolution, sizeof(resolution)))
    {
        if (DIAG_1) {
    ...
    Uns32 packedPixels = 0;
    if (bhmBoolAttribute("packedPixels")) {
    ...
    bhmCreateThread(refreshThread, 0, "refreshThread", &(stack[STACK_SIZE]));
}
