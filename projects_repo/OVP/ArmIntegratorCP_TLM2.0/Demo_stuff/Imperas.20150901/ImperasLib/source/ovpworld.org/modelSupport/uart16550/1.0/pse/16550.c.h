...
static Int32 writeOutput(Uns8 *buffer, Uns32 length) {
    ...
    if (state.channel >= 0) {
        ...
        if (rc != length) {
            if (!state.writeError) {
                bhmMessage("W", PREFIX"_CWE", "Serial channel write error. Channel closed?");
...
//
// Stack for the channel read thread
//
#define TOP (256 *1024)
...
static void initFifos() {
    ...
    if ((state.noFifo = bhmBoolAttribute("uart16450"))) {
        ...
        bhmIntegerAttribute("fifoSize", &fifoSize);
...
static void initBaudDelay() {
    ...
    if ((state.simulateBaud = bhmBoolAttribute("simulatebaud"))) {
        if (DIAG_LOW) {
    ...
    bhmIntegerAttribute("refClkFreq",  &state.refClkFreq);
