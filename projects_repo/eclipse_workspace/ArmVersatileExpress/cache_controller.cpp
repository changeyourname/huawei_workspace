...
void cache_controller::b_transport(int SocketId, tlm::tlm_generic_payload &payload, sc_core::sc_time &delay) {
	if (SocketId<4) {			// request from INSTRUCTION BUS
		...
		if (payload.get_command() == tlm::TLM_READ_COMMAND) {
			printf("(CC..IBUS..ACTUAL) addr:0x%08x\r\n", payload.get_address());
		}
	...
	if (SocketId>4 && SocketId<8) {		// request from DATA BUS
		...
		if (payload.get_command() == tlm::TLM_READ_COMMAND) {
			printf("(CC..DBUS..ACTUAL) addr:0x%08x\r\n", payload.get_address());
		}
...
unsigned int cache_controller::transport_dbg(int SocketId, tlm::tlm_generic_payload &payload) {
	if (SocketId<4) {			// request from INSTRUCTION BUS
		...
		static int count = 0;
		if (SocketId == 0 && payload.get_command() == tlm::TLM_READ_COMMAND) {
			printf("(CC..IBUS..DEBUG) addr:0x%08x\r\n", payload.get_address());
			count++;
			if (count == 100) {
				assert(0);
			}
	...
	if (SocketId>=4 && SocketId<8) {		// request from DATA BUS
		...
		if (payload.get_command() == tlm::TLM_READ_COMMAND) {
			printf("(CC..DBUS..DEBUG) addr:0x%08x\r\n", payload.get_address());
		}
