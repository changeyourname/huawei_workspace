...
icmStopReason icmProcessorObject::simulate(Uns64 instructions)
{
    return icmSimulate(_proc, instructions);
}
...
icmProcessorObject *icmProcessorObject::getSMPParent() {
    ...
    return (icmProcessorObject*)icmGetProcessorHandle(n);
}
...
icmProcessorObject *icmProcessorObject::find(const char *name) {
    ...
    if(proc) {
        pobj = (icmProcessorObject*)icmGetProcessorHandle(proc);
    }
    return pobj;
}
